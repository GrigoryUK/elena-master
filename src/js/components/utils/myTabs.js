export default class GraphTabsCustom {
  constructor(selector, nav, btn, panel, options) {
    let defaultOptions = {
      isChanged: () => {}
    }
    ...
    if (this.tabs) {
      this.tabList = this.tabs.querySelector(`${nav}`);
      this.tabsBtns = this.tabList.querySelectorAll(`${btn}`);
      this.tabsPanels = this.tabs.querySelectorAll(`${panel}`);
    } else {
